...
```sh
pnpm --filter @ariada/vscode-extension install
pnpm --filter @ariada/vscode-extension build
```
...
Rules that require a live DOM (contrast, focus order, computed styles) are out of scope for v0.1 and are addressed by the companion `@ariada/cli` runner.
