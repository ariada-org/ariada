...
in `@ariada/core-engine` — whether that is a way to crash analyzers through malformed snapshots, a fingerprint collision used to suppress a finding, a prototype-pollution vector through the registry, or anything else — please report it privately so we can fix it
...
- The package version you observed it in (`pnpm list @ariada/core-engine`).
...
- The published `@ariada/core-engine` npm package.
