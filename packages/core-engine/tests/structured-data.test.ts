...
//   5. Fixture-coverage harness — loads the authored fixture corpus from
//      packages/ariada-test-fixtures/fixtures/domains/structured-data/ and
//      asserts every fail-* case is flagged and every pass-* case is clean.
...
// ---------------------------------------------------------------------------
// 5. Fixture-coverage harness — authored corpus from ariada-test-fixtures
// ---------------------------------------------------------------------------

const __filename = fileURLToPath(import.meta.url);
const __fixtureDir = join(
  dirname(__filename),
  '../../ariada-test-fixtures/fixtures/domains/structured-data',
);

/** Load a fixture's HTML for a given rule directory. */
function loadFixtureHtml(ruleDir: string): string {
  return readFileSync(join(__fixtureDir, ruleDir, `${ruleDir}.html`), 'utf8');
}

/** Run the structured-data domain against fixture HTML and return findings keyed by ruleId. */
function fixtureFindings(ruleDir: string): Map<string, ReturnType<typeof structuredDataDomain.evaluate>> {
  const html = loadFixtureHtml(ruleDir);
  const features = featuresFromHtml(structuredDataDomain, html);
  const findings = structuredDataDomain.evaluate(features);
...
// ---- sd-parse-error fixture ----

describe('sd-parse-error fixture corpus', () => {
  it('fail-1: trailing-comma block triggers sd-parse-error at critical severity', () => {
    const byRule = fixtureFindings('sd-parse-error');
    const findings = byRule.get(RULE_PARSE_ERROR) ?? [];
    expect(findings.length).toBeGreaterThanOrEqual(1);
    expect(findings[0]?.severity).toBe('critical');
  });

  it('fail-2: truncated block also triggers sd-parse-error', () => {
    const byRule = fixtureFindings('sd-parse-error');
    const findings = byRule.get(RULE_PARSE_ERROR) ?? [];
    // Two invalid blocks (fail-1, fail-2) → at least 2 parse-error findings.
    expect(findings.length).toBeGreaterThanOrEqual(2);
  });

  it('pass-1: valid JSON-LD block produces no sd-parse-error finding', () => {
    // The fixture has exactly 3 blocks; only 2 are invalid. The third must not
    // produce a parse-error finding.
    const html = loadFixtureHtml('sd-parse-error');
    const features = featuresFromHtml(structuredDataDomain, html);
    const findings = structuredDataDomain.evaluate(features);
    const parseErrors = findings.filter((f) => f.ruleId === RULE_PARSE_ERROR);
    // Exactly 2 errors for the 2 invalid blocks — the valid one is skipped.
    expect(parseErrors).toHaveLength(2);
  });
});

// ---- sd-image-object-description fixture ----

describe('sd-image-object-description fixture corpus', () => {
  it('fail-1: ImageObject with contentUrl but no description triggers finding at moderate severity', () => {
    const byRule = fixtureFindings('sd-image-object-description');
    const findings = byRule.get(RULE_IMAGE_OBJECT_DESCRIPTION) ?? [];
    expect(findings.length).toBeGreaterThanOrEqual(1);
    expect(findings[0]?.severity).toBe('moderate');
  });

  it('fail-1 selector resolves to img[src="photo-product.jpg"] when contentUrl is present', () => {
    const byRule = fixtureFindings('sd-image-object-description');
    const findings = byRule.get(RULE_IMAGE_OBJECT_DESCRIPTION) ?? [];
    // fail-1 block has contentUrl="photo-product.jpg" → selector must use img[src=...] form.
    const fail1 = findings.find((f) => f.element.selector.includes('photo-product.jpg'));
    expect(fail1).toBeDefined();
  });

  it('fail-2: ImageObject with no contentUrl and no description triggers finding', () => {
    const byRule = fixtureFindings('sd-image-object-description');
    const findings = byRule.get(RULE_IMAGE_OBJECT_DESCRIPTION) ?? [];
    // Two failing blocks → at least 2 findings.
    expect(findings.length).toBeGreaterThanOrEqual(2);
  });

  it('pass-1: ImageObject with a description produces no sd-image-object-description finding', () => {
    // The fixture has 3 blocks; the third has description → no third finding.
    const html = loadFixtureHtml('sd-image-object-description');
    const features = featuresFromHtml(structuredDataDomain, html);
    const findings = structuredDataDomain.evaluate(features);
    const descFindings = findings.filter((f) => f.ruleId === RULE_IMAGE_OBJECT_DESCRIPTION);
    // Exactly 2 — for the two failing blocks only.
    expect(descFindings).toHaveLength(2);
  });
});

// ---- sd-product-required fixture ----

describe('sd-product-required fixture corpus', () => {
  it('fail-1: Product missing description produces sd-product-required at serious severity', () => {
    const byRule = fixtureFindings('sd-product-required');
    const findings = byRule.get(RULE_PRODUCT_REQUIRED) ?? [];
    expect(findings.length).toBeGreaterThanOrEqual(1);
    const descFinding = findings.find((f) => f.message.includes('"description"'));
    expect(descFinding).toBeDefined();
    expect(descFinding?.severity).toBe('serious');
  });

  it('fail-2: Product missing image, description, and offers produces multiple sd-product-required findings', () => {
    const byRule = fixtureFindings('sd-product-required');
    const findings = byRule.get(RULE_PRODUCT_REQUIRED) ?? [];
    // fail-1 (1 finding: description) + fail-2 (3 findings: image, description, offers) = at least 4.
    expect(findings.length).toBeGreaterThanOrEqual(4);
  });

  it('pass-1: fully populated Product produces no sd-product-required finding', () => {
    // The third block is complete; verify no product-required finding for it.
    // We do this by checking that the total number of product-required findings
    // matches only the failing blocks (fail-1: 1, fail-2: 3, pass-1: 0).
    const byRule = fixtureFindings('sd-product-required');
    const findings = byRule.get(RULE_PRODUCT_REQUIRED) ?? [];
    // fail-1 block index=0: missing description (1 finding).
    // fail-2 block index=1: missing image, description, offers (3 findings).
    // pass-1 block index=2: nothing missing → 0 findings.
    // Total must be exactly 4 product-required findings.
    expect(findings).toHaveLength(4);
  });

  it('pass-1 with future priceValidUntil produces no sd-price-validity finding', () => {
    const byRule = fixtureFindings('sd-product-required');
    const priceFindings = byRule.get(RULE_PRICE_VALIDITY) ?? [];
    // fail-1 has future priceValidUntil (2099-12-31) → no price finding for it.
    // fail-2 has no offers → no checkOffer call for it.
    // pass-1 has future priceValidUntil → no price finding.
    // Total price findings = 0.
    expect(priceFindings).toHaveLength(0);
  });
});

// ---- sd-price-validity fixture ----

describe('sd-price-validity fixture corpus', () => {
  it('fail-1: expired priceValidUntil produces sd-price-validity at critical severity', () => {
    const byRule = fixtureFindings('sd-price-validity');
    const findings = byRule.get(RULE_PRICE_VALIDITY) ?? [];
    expect(findings.length).toBeGreaterThanOrEqual(1);
    const expiredFinding = findings.find((f) => f.message.includes('has passed'));
    expect(expiredFinding).toBeDefined();
    expect(expiredFinding?.severity).toBe('critical');
  });

  it('fail-2: absent priceValidUntil produces sd-price-validity at critical severity', () => {
    const byRule = fixtureFindings('sd-price-validity');
    const findings = byRule.get(RULE_PRICE_VALIDITY) ?? [];
    const absentFinding = findings.find((f) => f.message.includes('missing'));
    expect(absentFinding).toBeDefined();
    expect(absentFinding?.severity).toBe('critical');
  });

  it('fail-1 and fail-2 together produce exactly 2 sd-price-validity findings', () => {
    const byRule = fixtureFindings('sd-price-validity');
    const findings = byRule.get(RULE_PRICE_VALIDITY) ?? [];
    expect(findings).toHaveLength(2);
  });

  it('pass-1: future priceValidUntil produces no sd-price-validity finding for that block', () => {
    // All 3 products have name, image, description so no sd-product-required.
    // fail-1 expired (1 finding), fail-2 absent (1 finding), pass-1 future (0).
    // Total: 2 price findings, already verified above.
    const byRule = fixtureFindings('sd-price-validity');
    const findings = byRule.get(RULE_PRICE_VALIDITY) ?? [];
    expect(findings).toHaveLength(2);
  });

  it('price-validity fixture produces no sd-product-required findings (all 3 products are otherwise complete)', () => {
    const byRule = fixtureFindings('sd-price-validity');
    const productFindings = byRule.get(RULE_PRODUCT_REQUIRED) ?? [];
    expect(productFindings).toHaveLength(0);
  });
});

// ---- sd-article-required fixture ----

describe('sd-article-required fixture corpus', () => {
  it('fail-1: Article missing image and having whitespace author.name produces sd-article-required findings', () => {
    const byRule = fixtureFindings('sd-article-required');
    const findings = byRule.get(RULE_ARTICLE_REQUIRED) ?? [];
    expect(findings.length).toBeGreaterThanOrEqual(1);
    // At least one finding for the missing image field.
    const imageFinding = findings.find(
      (f) => f.message.includes('"image"'),
    );
    expect(imageFinding).toBeDefined();
    expect(imageFinding?.severity).toBe('serious');
  });

  it('fail-1 also produces an author-name finding (whitespace-only author.name)', () => {
    const byRule = fixtureFindings('sd-article-required');
    const findings = byRule.get(RULE_ARTICLE_REQUIRED) ?? [];
    const authorFinding = findings.find((f) => f.message.includes('author.name'));
    expect(authorFinding).toBeDefined();
    expect(authorFinding?.severity).toBe('serious');
  });

  it('fail-2: BlogPosting missing headline, datePublished, image, and author produces 4 sd-article-required findings', () => {
    const byRule = fixtureFindings('sd-article-required');
    const findings = byRule.get(RULE_ARTICLE_REQUIRED) ?? [];
    // fail-1: 2 findings (image + author-name). fail-2: 4 findings (headline, datePublished, image, author-name).
    // Total: 6.
    expect(findings.length).toBeGreaterThanOrEqual(6);
  });

  it('pass-1: complete NewsArticle produces no sd-article-required finding', () => {
    // Verify exact count: only fail-1 (2) and fail-2 (4) findings, not pass-1.
    const byRule = fixtureFindings('sd-article-required');
    const findings = byRule.get(RULE_ARTICLE_REQUIRED) ?? [];
    expect(findings).toHaveLength(6);
  });

  it('all sd-article-required findings have serious severity', () => {
    const byRule = fixtureFindings('sd-article-required');
    const findings = byRule.get(RULE_ARTICLE_REQUIRED) ?? [];
    for (const f of findings) {
      expect(f.severity, `finding id=${f.id} must be serious`).toBe('serious');
    }
  });
});

// ---- Cross-fixture clean-page guard ----

describe('structured-data clean-page false-positive guard', () => {
  it('a minimal HTML page with no JSON-LD produces zero structured-data findings', () => {
    const features = featuresFromHtml(
      structuredDataDomain,
