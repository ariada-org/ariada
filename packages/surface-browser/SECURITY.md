...
in `@ariada-org/surface-browser` — whether that is a cross-origin analyzer
injection, a DOM-based data leak from the DevTools panel, a bookmarklet-scope
escape, or anything else — please report it privately so we can fix it before
it is publicised.
...
- The package version you observed it in (`pnpm list @ariada-org/surface-browser`).
...
- The published `@ariada-org/surface-browser` npm package.
