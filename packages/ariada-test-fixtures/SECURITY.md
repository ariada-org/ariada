...
in `@ariada-org/test-fixtures` — whether that is a hostile HTML fixture that
escapes the consumer's test harness, a golden-snapshot path-traversal vector,
a fixture that exfiltrates network metadata when loaded in a real browser,
or anything else — please report it privately so we can fix it before it is
publicised.
...
- A minimal reproduction (fixture filename, code snippet, or PoC).
- The package version you observed it in (`pnpm list @ariada-org/test-fixtures`).
...
- The published `@ariada-org/test-fixtures` npm package.
- The HTML fixtures under `fixtures/**` and any golden snapshots.
- The GitHub Actions workflows under `.github/workflows/**` (supply-chain).

Out of scope:

- Accessibility violations _intentionally embedded in fixtures_ — those are
  the fixtures' raison d'être, not security issues. Report only fixtures that
  attack the _consumer's test harness or browser_, not fixtures that
  legitimately fail an a11y check.
...
- Cosmetic issues with fixture content (those are bug reports, not security
  issues).
