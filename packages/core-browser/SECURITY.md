...
in `@ariada-org/core-browser` — whether that is a way to leak data through the captureBrowserSnapshot path, a way to evade the chrome.debugger isolation, or anything else — please report it privately so we can fix it
...
- The package version you observed it in (`pnpm list @ariada-org/core-browser`).
...
- The published `@ariada-org/core-browser` npm package.
