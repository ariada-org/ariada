...
async function listHtmlFiles(root: string): Promise<string[]> {
  ...
  // Отсортировано явным сравнением, а не голым `sort`. Без сравнителя порядок
  // идёт по кодовым единицам UTF-16: `B.html` встаёт перед `a.html`, и обход
  // выдаёт порядок, зависящий от регистра имён. Разбор качества считает это
  // ошибкой надёжности, и из-за неё эта перевозка дважды была отставлена.
