...
in `@ariada-org/brand-tokens` — whether that is a way to ship hostile CSS through
the published tokens, a stylesheet-import side-channel that leaks visitor
metadata, a token-name collision that could shadow downstream consumer
styles in a malicious way, or anything else — please report it privately so
we can fix it before it is publicised.
...
- A minimal reproduction (CSS token import, code snippet, or PoC).
- The package version you observed it in (`pnpm list @ariada-org/brand-tokens`).
...
- The published `@ariada-org/brand-tokens` npm package.
- The published CSS variable / token files under `tokens/**`.
...
- Aesthetic disagreements with token values (those are feature requests,
  not security issues).
