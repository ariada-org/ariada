...
function renderEvidenceReport(smoke, findings) {
  return page('S4 JetBrains Plugin Evidence Report', `
    ...
    <p>Ariada JetBrains plugin is a developer-IDE channel that runs the existing Ariada CLI scanner for a project URL and lists findings in an IDE tool window. This report starts with the channel description, then records market fit, implementation state, evidence, blockers, and distribution next steps.</p>

    <section><h2>Roles and payers</h2><table><thead><tr><th>Role</th><th>Job</th><th>Likely payer</th></tr></thead><tbody><tr><td>Frontend developer</td><td>Find WCAG issues before pull request review.</td><td>Engineering team lead or platform owner.</td></tr><tr><td>Accessibility champion</td><td>Coach teams inside IntelliJ IDEA, WebStorm, and related IDEs.</td><td>Compliance or quality owner.</td></tr><tr><td>Agency delivery team</td><td>Run repeatable checks on client projects before handoff.</td><td>Agency operations or client retainer.</td></tr></tbody></table></section>
    ...
    <section><h2>Implemented vs missing</h2><table><thead><tr><th>Area</th><th>Status</th></tr></thead><tbody><tr><td>Gradle IntelliJ scaffold</td><td>Implemented: Java plugin, plugin.xml, buildPlugin task.</td></tr><tr><td>Action and tool window</td><td>Implemented: Tools menu action and Ariada findings panel.</td></tr><tr><td>Scanner reuse</td><td>Implemented: shells out to Ariada CLI, expects HTTP(S) project URL from environment, .ariada-url, or prompt.</td></tr><tr><td>Open-file mapping</td><td>Missing: no source-line mapping from finding to editor range yet.</td></tr><tr><td>Marketplace readiness</td><td>Blocked on founder JetBrains account, signing certificate, verifier matrix, icon and listing copy.</td></tr></tbody></table></section>
