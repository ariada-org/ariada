...
def default_options(**overrides: object) -> ScanOptions:
    ...
    cli_command = str(
        overrides.get("cli_command") or getattr(settings, "ARIADA_CLI_COMMAND", "ariada")
    )
    domains_raw = overrides.get("domains", getattr(settings, "ARIADA_SCAN_DOMAINS", ()))
    domains = tuple(domains_raw or ())
    return ScanOptions(
        output_dir=output_dir,
        cli_command=cli_command,
        browser=str(overrides.get("browser", getattr(settings, "ARIADA_SCAN_BROWSER", "chromium"))),
        format=str(overrides.get("format", "json")),
        severity_threshold=str(
            overrides.get(
                "severity_threshold",
                getattr(settings, "ARIADA_SCAN_SEVERITY_THRESHOLD", "moderate"),
            )
        ),
        timeout_ms=int(
            overrides.get("timeout_ms", getattr(settings, "ARIADA_SCAN_TIMEOUT_MS", 30_000))
        ),
        domains=domains,
    )
...
def configured_targets() -> list[str]:
    return [str(target) for target in getattr(settings, "ARIADA_SCAN_TARGETS", [])]
...
def scan_target(
    ...
    html = render_target_to_html(target)
...
def render_target_to_html(target: str) -> bytes:
    path = Path(target)
    if path.exists() and path.is_file():
        return path.read_bytes()

    django_path = target if target.startswith("/") else f"/{target}"
    response = Client().get(django_path)
    if response.status_code >= 400:
        raise ValueError(f"Django path {django_path} returned HTTP {response.status_code}")
    return bytes(response.content)


class ServedHtml(AbstractContextManager[str]):
    def __init__(self, html: bytes) -> None:
        self._tmp = tempfile.TemporaryDirectory(prefix="ariada-django-")
