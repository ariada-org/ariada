...
import { asText, toNumber, type AdminGridRow } from './format';
...
/** one drawn bar: which series it belongs to and where it sits. */
...
/** one group along the category axis, with its bars and its hover band. */
...
/** everything a column, line or funnel chart needs to draw itself. */
...
const label = (value: unknown): string => asText(value) || '—';
...
/** a node of a relationship graph, already placed. */
...
/** an edge of a relationship graph, as the line that will be drawn. */
...
/** everything a relationship graph needs to draw itself. */
