...
/**
 * Strip the anchor comments the server renderer leaves behind, until none are
 * left. One pass is not enough: a comment removed from the middle can bring the
 * text on either side of it together into a new one, so a single replace can
 * hand back a string that still carries the thing it was asked to remove.
 */
function withoutMarkupComments(html: string): string {
  let out = html;
  let previous: string;
  do {
    previous = out;
...
describe('<RowDetailDrawer> server render', () => {
  ...
  it('renders nothing when no row is open', () => {
    ...
    expect(withoutMarkupComments(body).trim()).toBe('');
  });
