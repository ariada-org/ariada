...
describe('colour ramp', () => {
  it('interpolates red -> green and clamps outside 0..1', () => {
    ...
    expect(rampColor(1).fg).toBe('rgb(4,108,78)');
  ...
  it('emits a translucent chip background alongside the foreground', () => {
    expect(rampColor(1).bg).toBe('rgba(4,108,78,0.12)');
  });
  ...
  it('resolves the ramp variant from the CONTRACT, never from a column name', () => {
    ...
    expect(rampVariant({ kind: 'score', colorRamp: { good: 'high' } })).toBe('score');
  });
  ...
  it('renders each ramp variant on its own scale', () => {
    ...
    // a 0..10 score renders as a plain integer, never a signed delta
    expect(rampContent('score', 1).text).toBe('1');
    expect(rampContent('score', 10).text).toBe('10');
  });
});
...
describe('status and tag palettes', () => {
  ...
  it('maps tags and falls back to slate', () => {
    ...
    expect(tagColor('mystery')).toBe('#475569');
  });
});
...
describe('wiki links', () => {
  it('uses the column key when the contract omits a slug', () => {
    expect(wikiHref(DEFAULT_WIKI, { description: 'x' }, 'owedRatio'))
      .toBe('https://wiki.ariada.org/en/metrics/owedRatio/');
  });

  it('honours slug, anchor, language and a trailing-slash base', () => {
    expect(wikiHref({ base: 'https://wiki.ariada.org/', lang: 'ru' }, { description: 'x', wikiSlug: 'owed-ratio', wikiAnchor: 'formula' }, 'owedRatio'))
      .toBe('https://wiki.ariada.org/ru/metrics/owed-ratio/#formula');
  });
});
...
describe('a value that has no text form', () => {
  // The values reaching these functions are typed `unknown` because they come
  // out of a row, and a row is whatever the consumer put in it. `String()` on
  // an object writes "[object Object]" into the cell, which looks like data and
  // is not; every one of these has a fallback of its own that is better.
  it('comes back empty rather than as the name of its type', () => {
    expect(asText({})).toBe('');
    expect(asText([1, 2])).toBe('');
    expect(asText(() => 1)).toBe('');
    expect(asText(null)).toBe('');
    expect(asText(undefined)).toBe('');
  });

  it('keeps the ordinary values it was always given', () => {
    expect(asText('alpha')).toBe('alpha');
    expect(asText(42)).toBe('42');
    expect(asText(false)).toBe('false');
  });

  it('lets each caller fall back to what it meant to show', () => {
    expect(formatByKind({}, undefined)).toBe('');
    expect(escapeHtml({})).toBe('');
    expect(rowLabel({ id: {} } as never)).toBe('');
    expect(tagColor({})).toBe(tagColor('nothing-known'));
    expect(statusColor({})).toBe(statusColor('nothing-known'));
  });
});
