...
All notable changes to `@ariada/ai-authorship` will be documented here. The
...
- Composition with `@ariada/haes` — `anchorPosterior` canonicalises (RFC 8785 JCS), builds a HAES payload, signs the entry hash with the caller's Ed25519 keypair, and appends to a chain. `buildAnchorInclusionProof` produces a Merkle inclusion proof against the daily anchor.
