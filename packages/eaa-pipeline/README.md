...
Reusable GitHub Actions workflow that scans public URLs with `@ariada/wcag-rules-extended`, generates an accessibility statement, and uploads VPAT-JSON evidence as build artefacts.
...
A single [reusable workflow](https://docs.github.com/en/actions/sharing-automations/reusing-workflows) (`eaa-audit.yml`) any GitHub Actions caller invokes with one `uses:` line. The workflow installs Node 22 + pnpm + axe-core + `@ariada/wcag-rules-extended` into a scratch project, scans each URL in `pages`, and uploads a single artefact bundle: a JSON violation report, an optional HTML accessibility statement, and an optional VPAT-JSON + `.well-known/accessibility.json` pair.
...
| `pack-version` | no | `next` | npm dist-tag or semver of `@ariada/wcag-rules-extended`. |
...
- WCAG 2.2 AA — W3C Recommendation (2023-10-05), implemented via `@ariada/wcag-rules-extended`
...
- [`@ariada/wcag-rules-extended`](../wcag-rules-extended) — the rule pack the workflow runs
- [`@ariada/evidence-emitter`](../ariada-evidence-emitter) — emits the VPAT / EN 301 549 / DOS-lagen JSON
- [`@ariada/statement-generator`](../ariada-statement-generator) — renders the accessibility statement
