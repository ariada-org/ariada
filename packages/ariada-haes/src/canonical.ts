...
function encodeObject(obj: Record<string, unknown>, seen: Set<object>): string {
  ...
  seen.add(obj);
  try {
    // RFC 8785 §3.2.3: sort keys by UTF-16 code-unit order. The explicit
    // comparator matches the JS default-sort behaviour for strings while
    // making the contract loud (and silences ambient-sort-comparator lint).
