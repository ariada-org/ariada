...
describe('checkout/order-confirmation-focus — check', () => {
  ...
  it('FAILS when confirmation h1 has no live region and no tabindex', () => {
    const document = setBodyFromFragment(`<h1>Order confirmed!</h1>`);
    expect(check(document.querySelector('h1')!)).toBe(false);
  });

  it('PASSES when h1 has role=status', () => {
    const document = setBodyFromFragment(`<h1 role="status">Order placed</h1>`);
    expect(check(document.querySelector('h1')!)).toBe(true);
  });

  it('PASSES when h1 has tabindex=-1', () => {
    const document = setBodyFromFragment(`<h1 tabindex="-1">Thank you for your order</h1>`);
    expect(check(document.querySelector('h1')!)).toBe(true);
  });
  ...
  it('PASSES on Swedish "Tack för din beställning" with role=status', () => {
    const document = setBodyFromFragment(`<h1 role="status">Tack för din beställning</h1>`);
    expect(check(document.querySelector('h1')!)).toBe(true);
  });

  it('SKIPS non-confirmation h1s', () => {
    const document = setBodyFromFragment(`<h1>Welcome to our shop</h1>`);
    expect(check(document.querySelector('h1')!)).toBe(true);
  });
  ...
  it('SKIPS h2 / h3 confirmation headings (rule scoped to <h1> only)', () => {
    ...
    const document = setBodyFromFragment(`<h2>Order confirmed!</h2>`);
    // h2 fails the isConfirmationHeading tag check → returns true (skipped).
    expect(check(document.querySelector('h2')!)).toBe(true);
  });
  ...
  it('PASSES Norwegian "Takk for din bestilling" with tabindex=-1', () => {
    const document = setBodyFromFragment(`<h1 tabindex="-1">Takk for din bestilling</h1>`);
    expect(check(document.querySelector('h1')!)).toBe(true);
  });

  it('PASSES Danish "Tak for din ordre" with role=status', () => {
    const document = setBodyFromFragment(`<h1 role="status">Tak for din ordre</h1>`);
    expect(check(document.querySelector('h1')!)).toBe(true);
  });

  it('FAILS Swedish confirmation h1 with no live region and no tabindex', () => {
    const document = setBodyFromFragment(`<h1>Tack för din beställning</h1>`);
    expect(check(document.querySelector('h1')!)).toBe(false);
  });
  ...
  it('PASSES with tabindex="0" on confirmation h1 (rule now accepts any non-positive tabindex; rejects only positive anti-pattern values)', () => {
    ...
    const document = setBodyFromFragment(`<h1 tabindex="0">Order placed successfully</h1>`);
    expect(check(document.querySelector('h1')!)).toBe(true);
  });

  it('SKIPS h1 without confirmation tokens regardless of attributes', () => {
    const document = setBodyFromFragment(`<h1>Shop home</h1>`);
    expect(check(document.querySelector('h1')!)).toBe(true);
  });
  ...
  it('PASSES h1 with role=alert (assertive live region)', () => {
    const document = setBodyFromFragment(`<h1 role="alert">Order placed</h1>`);
    expect(check(document.querySelector('h1')!)).toBe(true);
  });
