...
describe('checkout/cart-quantity-input-label — check', () => {
  ...
  it('FAILS when label is generic "Qty"', () => {
    ...
    `);
    expect(check(doc.querySelector('#q1')!)).toBe(false);
  });
  ...
  it('SKIPS non-quantity inputs', () => {
    const doc = setBodyFromFragment(`<input type="text" name="search" value="">`);
    expect(check(doc.querySelector('input')!)).toBe(true);
  });
  ...
  it('SKIPS quantity-named input that is type=search (not numeric quantity)', () => {
    const doc = setBodyFromFragment(`<input type="search" name="qty-search">`);
    expect(check(doc.querySelector('input')!)).toBe(true);
  });
