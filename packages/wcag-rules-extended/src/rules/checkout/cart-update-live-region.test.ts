...
describe('checkout/cart-update-live-region — check', () => {
  ...
  it('FAILS when cart region has no live-region attribute', () => {
    const document = setBodyFromFragment(`<div id="cart-summary"><span>Items: 3</span></div>`);
    expect(check(document.querySelector('#cart-summary')!)).toBe(false);
  });
  ...
  it('PASSES when cart region has aria-live=polite', () => {
    ...
    expect(check(document.querySelector('#cart-summary')!)).toBe(true);
  });

  it('PASSES when cart region has role=status', () => {
    const document = setBodyFromFragment(`<div class="cart" role="status">Items: 3</div>`);
    expect(check(document.querySelector('.cart')!)).toBe(true);
  });

  it('PASSES when cart region has role=alert', () => {
    const document = setBodyFromFragment(`<div data-role="basket" role="alert">Items: 3</div>`);
    expect(check(document.querySelector('[data-role="basket"]')!)).toBe(true);
  });
  ...
  it('SKIPS non-cart elements', () => {
    const document = setBodyFromFragment(`<div id="hero">Welcome</div>`);
  ...
  it('PASSES when cart region uses role="log" (less common but valid live region)', () => {
    const document = setBodyFromFragment(`<div id="cart-summary" role="log">3 items</div>`);
    expect(check(document.querySelector('#cart-summary')!)).toBe(true);
  });
  ...
  it('PASSES Swedish "varukorg" class with role=status', () => {
    const document = setBodyFromFragment(`<div class="varukorg" role="status">3 varor</div>`);
    expect(check(document.querySelector('.varukorg')!)).toBe(true);
  });

  it('PASSES Finnish "ostoskori" id with aria-live', () => {
    const document = setBodyFromFragment(`<div id="ostoskori" aria-live="polite">3 tuotetta</div>`);
    expect(check(document.querySelector('#ostoskori')!)).toBe(true);
  });

  it('PASSES aria-live="assertive" on cart region', () => {
    const document = setBodyFromFragment(`<div id="cart" aria-live="assertive">3 items</div>`);
    expect(check(document.querySelector('#cart')!)).toBe(true);
  });

  it('FAILS cart region with aria-live="" empty string', () => {
    const document = setBodyFromFragment(`<div id="cart-summary" aria-live="">3 items</div>`);
    expect(check(document.querySelector('#cart-summary')!)).toBe(false);
  });
  ...
  it('PASSES empty cart region with role=status (zero items still announced)', () => {
    const document = setBodyFromFragment(`<div id="cart-summary" role="status"></div>`);
    expect(check(document.querySelector('#cart-summary')!)).toBe(true);
  });

  it('SKIPS hero element with no cart-pattern class', () => {
    const document = setBodyFromFragment(`<div class="promo-banner">Save 20%</div>`);
    expect(check(document.querySelector('.promo-banner')!)).toBe(true);
  });
