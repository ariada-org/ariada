...
describe('banking/iban-input-format — check', () => {
  ...
  it('FAILS without format hint', () => {
    const document = setBodyFromFragment(`<input type="text" name="iban" aria-label="IBAN">`);
  ...
  it('SKIPS non-IBAN inputs', () => {
    const document = setBodyFromFragment(`<input type="text" name="account">`);
    expect(check(document.querySelector('input')!)).toBe(true);
  });
  ...
  it('FAILS IBAN with emoji-only aria-label (no "IBAN" word match)', () => {
    ...
    expect(check(document.querySelector('input')!)).toBe(true); // skips, not IBAN
  });
