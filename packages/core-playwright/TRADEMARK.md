...
This document records the trademark posture of `@ariada-org/core-playwright` as an open-source
...
- Domain names that could create consumer confusion (e.g. `ariada-foo.com`, `core-playwright-pro.com`, `core-playwright-cloud.com`, `core-playwright-official.com`);
- Trademark applications in any jurisdiction.

The EUPL-1.2 licence under which the Playwright-adapter TypeScript code is distributed grants copyright
permissions only — it does not transfer trademark rights. The brand-asset
SVG files and Rive `.riv` character files are intentionally left in the
private monorepo precisely because they are trademark-restricted and would
create downstream brand confusion if mistakenly re-distributed.
...
- Reference `@ariada-org/core-playwright` by package name in their dependency manifest.

Forks of this package MAY:

- Credit «Originally derived from `@ariada-org/core-playwright`» as historical attribution;
- Reference the public EUPL-1.2 licence terms.

Forks of this package MAY NOT:

- Use any reserved mark in the fork name;
...
If you fork this package to ship a derivative product, please re-name the
package and any brand-identifier exports so downstream users do not mistake
your product for an Agonist Development AB product.
...
permissionless copyright remains intact via the EUPL-1.2 licence.
...
_Document version: 0.1 (2026-05-19). Public trademark policy of Agonist Development AB._
