...
```bash
# One-off via npx
npx @ariada/cli scan https://example.com

# Global install
npm install -g @ariada/cli
ariada scan https://example.com

# Workspace dev
pnpm --filter @ariada/cli build
...
Print every rule registered by `@ariada/wcag-rules-extended`.
...
```ts
import { run, runScan, runListRules } from '@ariada/cli';
