...
import { ariadaRollup, scanBundle, scanDirectory, type HtmlScanner } from '../src/index.js';
...
describe('@ariada-org/rollup-plugin', () => {
  it('orders the files it finds by comparison, not by code unit', async () => {
    ...
    // встаёт перед строчной. Разбор качества считает голый `sort` ошибкой
    // надёжности — из-за неё эта перевозка дважды была отставлена.
    const root = await mkdtemp(join(tmpdir(), 'ariada-rollup-order-'));
    try {
