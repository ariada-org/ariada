...
export interface UxpinAriadaConfig {
  exportDir?: string;
  targetUrl?: string;
  outputDir?: string;
  browser?: BrowserName;
...
export interface RunUxpinScanOptions {
  cwd?: string;
  cliCommand?: string;
  runner?: CliRunner;
}

export interface UxpinScanResult extends RunnerResult {
  commandLine: string;
  targetUrl: string;
  servedExportDir?: string;
}
...
export function validateConfig(config: UxpinAriadaConfig): string[] {
  const errors: string[] = [];
  if (!config.exportDir && !config.targetUrl) {
    errors.push('Set exportDir for a local UXPin HTML export or targetUrl for an already hosted preview.');
  }
  if (config.exportDir && config.targetUrl) {
    errors.push('Use either exportDir or targetUrl, not both.');
  }
...
export async function loadConfig(path: string): Promise<UxpinAriadaConfig> {
  const body = await readFile(path, 'utf8');
  return JSON.parse(body) as UxpinAriadaConfig;
}

export async function findUxpinExportOutput(
  startDir: string,
  ...
  async function visit(dir: string, depth: number): Promise<void> {
    const discovered = await inspectExportDir(dir, options.entryFile);
  ...
  if (!best || best.score < 3) {
    throw new Error(`No UXPin HTML export found under ${root}. Expected index.html plus UXPin export markers.`);
  }
  return best;
}

export function buildAriadaCliArgs(targetUrl: string, config: UxpinAriadaConfig): string[] {
...
export async function runUxpinScan(
  config: UxpinAriadaConfig,
  options: RunUxpinScanOptions = {},
): Promise<UxpinScanResult> {
  ...
  let servedExportDir: string | undefined;

  try {
    if (!targetUrl) {
      const exportRoot = resolve(cwd, config.exportDir ?? '.');
      const discovered = await findUxpinExportOutput(exportRoot, { entryFile: config.entryFile });
      const served = await serveStatic(discovered.dir);
      closeServer = served.close;
      servedExportDir = discovered.dir;
    ...
    return {
      ...
      targetUrl,
      ...(servedExportDir ? { servedExportDir } : {}),
    };
  } finally {
    await closeServer?.();
  }
...
async function inspectExportDir(dir: string, entryFile = 'index.html'): Promise<DiscoveredExport | undefined> {
  const markers: string[] = [];
  const entry = join(dir, entryFile);
  if (!(await fileExists(entry))) return undefined;

  const markerPaths = ['assets/uxpin-export.json', 'assets/uxpin-preview.js', 'assets/uxpin-components.css'];
  ...
  if (/UXPin|uxpin|data-uxpin|uxpin-preview|uxpin-merge/iu.test(html)) {
    markers.push(`${entryFile}:uxpin-html-marker`);
  }
...
async function serveStatic(root: string): Promise<{ baseUrl: string; close: () => Promise<void> }> {
  ...
  if (!address || typeof address === 'string') throw new Error('Could not bind local UXPin export server.');
  return {
