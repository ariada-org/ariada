...
def default_options(app: Flask, **overrides: object) -> ScanOptions:
    output_dir_value = overrides.get("output_dir") or app.config.get(
        "ARIADA_SCAN_OUTPUT_DIR",
        "ariada-output",
    )
    domains_raw = overrides.get("domains", app.config.get("ARIADA_SCAN_DOMAINS", ()))
    return ScanOptions(
        output_dir=Path(str(output_dir_value)),
        cli_command=str(
            overrides.get("cli_command") or app.config.get("ARIADA_CLI_COMMAND", "ariada")
        ),
        browser=str(overrides.get("browser", app.config.get("ARIADA_SCAN_BROWSER", "chromium"))),
        format=str(overrides.get("format", "json")),
        severity_threshold=str(
            overrides.get(
                "severity_threshold",
                app.config.get("ARIADA_SCAN_SEVERITY_THRESHOLD", "moderate"),
            )
        ),
        timeout_ms=int(
            overrides.get("timeout_ms", app.config.get("ARIADA_SCAN_TIMEOUT_MS", 30_000))
        ),
        domains=tuple(domains_raw or ()),
    )


def configured_targets(app: Flask) -> list[str]:
    return [str(target) for target in app.config.get("ARIADA_SCAN_TARGETS", [])]
...
def render_target_to_html(app: Flask, target: str) -> bytes:
    path = Path(target)
    if path.exists() and path.is_file():
        return path.read_bytes()

    flask_path = target if target.startswith("/") else f"/{target}"
    response = app.test_client().get(flask_path)
    if response.status_code >= 400:
        raise ValueError(f"Flask path {flask_path} returned HTTP {response.status_code}")
    return bytes(response.data)


class ServedHtml(AbstractContextManager[str]):
    def __init__(self, html: bytes) -> None:
        self._tmp = tempfile.TemporaryDirectory(prefix="ariada-flask-")
