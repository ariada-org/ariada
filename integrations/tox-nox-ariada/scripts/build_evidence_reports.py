...
def build_test_report() -> None:
    gates = [
        ...
        ("ruff", "ruff check ."),
        ("pytest", "pytest -q"),
        ("compileall", "python -m compileall -q tox_nox_ariada tests"),
        ("build", "python -m build"),
        ("ariada-cli-build", "pnpm --filter @ariada-org/cli build"),
        ("scan", "ariada-toxnox scan examples/site/index.html"),
    ]
    rows = "\n".join(
    ...
    (TEST_REPORT / "result.html").write_text(
        page(
            ...
            f"<p>Focused local gates for the tox/nox helper.</p><table><tbody>{rows}</tbody></table><h2>Logs</h2>{logs}",
        ),
        encoding="utf-8",
    )
...
def build_scan_preview() -> None:
    ...
    (SCAN_EVIDENCE / "scan-result-preview.html").write_text(
        page(
            ...
            f"""
<p>Real Ariada CLI scan triggered through <code>ariada-toxnox scan examples/site/index.html</code>.</p>
...
def build_scan_report() -> None:
    ...
    if screenshot.exists():
        ...
        shot = (
            "<figure><img alt='Screenshot of the Ariada tox/nox scan result' "
    ...
    (SCAN_EVIDENCE / "result.html").write_text(
        page(
            ...
            f"""
<p>Representative host surface: generated HTML fixture that a tox/nox job can produce.</p>
<p>Scanner path: tox/nox helper CLI to <code>@ariada-org/cli</code>.</p>
...
<h2>Host Blockers</h2>
<p>PyPI publication and running in a private project CI require founder-owned credentials or repository access. Local file-surface evidence is complete.</p>
""",
        ),
        encoding="utf-8",
    )
