...
from dash_ariada.component import render_summary
from dash_ariada.cli import main
from dash_ariada.scanner import AriadaScanOptions, count_findings, scan_url
...
def test_cli_returns_zero_with_no_fail_and_json_output(tmp_path: Path, monkeypatch) -> None:  # type: ignore[no-untyped-def]
    def fake_scan_url(app_url, options):  # type: ignore[no-untyped-def]
        from dash_ariada.scanner import AriadaScanResult

        return AriadaScanResult(app_url, 0, "", "", tmp_path / "report.json", 3)

    monkeypatch.setattr("dash_ariada.cli.scan_url", fake_scan_url)
    assert main(["scan", "http://localhost:8050", "--json", "--no-fail"]) == 0
...
def test_render_summary_returns_dash_status_component(monkeypatch) -> None:  # type: ignore[no-untyped-def]
    class FakeHtml:
        @staticmethod
        def Div(children, **props):  # type: ignore[no-untyped-def]
            return ("Div", children, props)

        @staticmethod
        def Small(text):  # type: ignore[no-untyped-def]
            return ("Small", text)

        @staticmethod
        def Span(text, **props):  # type: ignore[no-untyped-def]
            return ("Span", text, props)

        @staticmethod
        def Strong(text):  # type: ignore[no-untyped-def]
            return ("Strong", text)

    monkeypatch.setitem(sys.modules, "dash", types.SimpleNamespace(html=FakeHtml))

    component = render_summary({"totalFindings": 3, "reportPath": "ariada-output/report.json"})

    assert component[0] == "Div"
    assert component[2]["role"] == "status"
