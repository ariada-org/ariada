...
def build_scan_report() -> None:
    ...
    domain_expansion_rows = "\n".join(
        [
            ...
            "<tr><th scope='row'>Не выдавать за готовое</th><td><code>performance</code> пока не отдельный built-in domain в текущей fixtures/PRD карте.</td><td>P0 использует performance как пример cross-domain interaction, но текущая initial-domain set: accessibility, privacy, security, ai-readiness, structured-data, sustainability.</td><td>Если нужен performance domain, нужен отдельный PRD/package/fixture set, а не строка в отчете.</td></tr>",
        ]
    )
    monetization_rows = "\n".join(
        [
...
и P0/P1-P6 PRD. Для Dash важно расширяться не “по красоте”, а по buyer pain: что быстрее блокирует release или создает платный enterprise use-case.</p>
