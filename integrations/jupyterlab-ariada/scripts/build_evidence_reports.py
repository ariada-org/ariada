...
def scan_total(report: dict) -> int:
    ...
    if not isinstance(grid, dict):
        return int(report.get("summary", {}).get("total", 0)) if isinstance(report.get("summary"), dict) else 0
    total = 0
...
def build_test_report() -> None:
    gates = [
        ...
        ("ruff", "ruff check ."),
        ("pytest", "pytest -q"),
        ("compileall", "python -m compileall -q jupyterlab_ariada tests"),
    ...
    (TEST_REPORT / "result.html").write_text(
        page(
            ...
            f"<p>Focused local gates for the JupyterLab bridge.</p><table><tbody>{rows}</tbody></table><h2>Logs</h2>{logs}",
        ),
        encoding="utf-8",
    )
...
def build_scan_preview() -> None:
    ...
    (SCAN_EVIDENCE / "scan-result-preview.html").write_text(
        page(
            ...
            f"""
<p>Real Ariada CLI scan triggered through <code>python -m jupyterlab_ariada examples/fixture-notebook.ipynb</code>.</p>
...
def build_scan_report() -> None:
    ...
    if screenshot.exists():
        ...
        shot = (
            "<figure><img alt='Screenshot of the Ariada JupyterLab scan result' "
    ...
    (SCAN_EVIDENCE / "result.html").write_text(
        page(
            ...
            f"""
<p>Representative host surface: a notebook exported through nbconvert to HTML.</p>
<p>Scanner path: JupyterLab bridge to temporary localhost HTML to <code>@ariada-org/cli</code>.</p>
...
<h2>Host Blockers</h2>
<p>PyPI publication, npm labextension publication, and a live JupyterLab demo require founder-owned accounts. Local notebook-export surface evidence is complete.</p>
""",
        ),
        encoding="utf-8",
    )
