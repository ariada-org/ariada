...
export function findEntryHtml(targetDir) {
  ...
  throw new Error(`No HTML files found under ${root}. Run zola build before zola-ariada.`);
}
...
function usage() {
  return `Usage: zola-ariada [options]

Options:
  --target-dir <dir>            Built Zola output directory, normally public
...
if (import.meta.url === pathToFileURL(process.argv[1]).href) {
  try {
    ...
    process.exit(result.gateFailed ? 1 : 0);
  } catch (error) {
