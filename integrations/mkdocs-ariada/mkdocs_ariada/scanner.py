...
def scan_mkdocs_site(
    ...
    if not index.exists():
        ...
        if not html_files:
            raise FileNotFoundError(f"No HTML files found under {site_dir}")
        index = html_files[0]

    options.output_dir.mkdir(parents=True, exist_ok=True)
    with ServedDirectory(site_dir) as base_url:
        relative = index.relative_to(site_dir).as_posix()
