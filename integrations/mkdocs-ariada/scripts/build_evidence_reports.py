...
def build_test_report() -> None:
    gates = [
        ...
        ("ruff", "ruff check ."),
        ("pytest", "pytest -q"),
        ("compileall", "python -m compileall -q mkdocs_ariada tests"),
        ("build", "python -m build"),
        ("mkdocs", "mkdocs build -f examples/docs/mkdocs.yml -d scan-evidence/site"),
    ]
    rows = "\n".join(
    ...
    (TEST_REPORT / "result.html").write_text(
        page(
            ...
            f"<p>Focused local gates for the MkDocs plugin.</p><table><tbody>{rows}</tbody></table><h2>Logs</h2>{logs}",
        ),
        encoding="utf-8",
    )
...
def build_scan_preview() -> None:
    ...
    (SCAN_EVIDENCE / "scan-result-preview.html").write_text(
        page(
            ...
            f"""
<p>Real Ariada CLI scan triggered through <code>mkdocs build -f examples/docs/mkdocs.yml -d scan-evidence/site</code>.</p>
...
def build_scan_report() -> None:
    ...
    if screenshot.exists():
        ...
        shot = (
            "<figure><img alt='Screenshot of the Ariada MkDocs scan result' "
    ...
    (SCAN_EVIDENCE / "result.html").write_text(
        page(
            ...
            f"""
<p>Representative host surface: MkDocs-generated HTML from a fixture docs project.</p>
<p>Scanner path: MkDocs <code>on_post_build</code> hook to temporary localhost HTML to <code>@ariada-org/cli</code>.</p>
...
<h2>Host Blockers</h2>
<p>PyPI publication requires founder-owned credentials. Local MkDocs build and scan evidence is complete.</p>
""",
        ),
        encoding="utf-8",
    )
