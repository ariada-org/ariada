...
function renderLayer(snapshot, layer) {
...
export function renderZeplinScanTarget(snapshot) {
...
export function buildZeplinExtensionSnippet(snapshot) {
...
function findings(envelope) {
    const found = envelope.report?.findings;
    return !found ? [] : Array.isArray(found) ? [...found] : Object.values(found).flat();
}
export function summarizeAriadaScan(snapshot, envelope) {
    if (!envelope)
        return { title: `Ariada evidence for ${snapshot.screenName}`, status: 'needs-scan', totalFindings: 0, contrastFindings: 0, copy: ['Run the exported fixture through @ariada-org/cli before treating this Zeplin panel as an accessibility verdict.'] };
