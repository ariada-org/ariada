...
def build_test_report() -> None:
    gates = [
        ...
        ("ruff", "ruff check ."),
        ("pytest", "pytest -q"),
        ("compileall", "python -m compileall -q ariada_django tests"),
    ...
    html_out = page(
        ...
        f"""
<p>Focused local gates for the Django adapter package.</p>
<table><thead><tr>
...
def build_scan_preview() -> None:
    ...
    body = f"""
<p>Real Ariada CLI scan triggered through <code>python manage.py ariada_scan /broken/</code>.</p>
...
"""
    ...
    (SCAN_EVIDENCE / "scan-result-preview.html").write_text(
        page("Ariada Django real scan preview", body),
        encoding="utf-8",
    )
...
def build_scan_report() -> None:
    ...
    if screenshot.exists():
        ...
        shot = (
            "<figure><img alt='Screenshot of the Ariada Django scan result' "
    ...
    body = f"""
<p>Representative host surface: a minimal Django project rendered through the Django
test client.</p>
<p>Scanner path: Django management command to temporary localhost HTML to
...
and a deployed Django site. Local host-surface evidence is complete.</p>
"""
    (SCAN_EVIDENCE / "result.html").write_text(
        page("Ariada Django scan evidence", body),
        encoding="utf-8",
    )
