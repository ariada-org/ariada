...
def build_test_report() -> None:
    gates = [
        ...
        ("ruff", "ruff check ."),
        ("pytest", "pytest -q"),
        ("compileall", "python -m compileall -q streamlit_ariada tests"),
        ("build", "python -m build"),
        ("scan", "streamlit-ariada scan http://127.0.0.1:<fixture-port>"),
    ]
    rows = "\n".join(
    ...
    (TEST_REPORT / "result.html").write_text(
        page(
            ...
            f"<p>Focused local gates for the Streamlit helper.</p><table><tbody>{rows}</tbody></table><h2>Logs</h2>{logs}",
        ),
        encoding="utf-8",
    )
...
def build_scan_preview() -> None:
    ...
    (SCAN_EVIDENCE / "scan-result-preview.html").write_text(
        page(
            ...
            f"""
<p>Real Ariada CLI scan triggered through <code>streamlit-ariada scan http://127.0.0.1:&lt;fixture-port&gt;</code>.</p>
...
def build_scan_report() -> None:
    ...
    if screenshot.exists():
        ...
        shot = (
            "<figure><img alt='Screenshot of the Ariada Streamlit scan result' "
    ...
    (SCAN_EVIDENCE / "result.html").write_text(
        page(
            ...
            f"""
<p>Representative host surface: served Streamlit-like HTML fixture for the helper CLI.</p>
<p>Scanner path: Streamlit helper CLI to <code>@ariada-org/cli</code>.</p>
...
<h2>Host Blockers</h2>
<p>PyPI publication and scanning a deployed Streamlit Cloud app require founder-owned account access. Local served-surface evidence is complete.</p>
""",
        ),
        encoding="utf-8",
    )
