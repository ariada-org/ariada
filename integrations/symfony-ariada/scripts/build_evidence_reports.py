...
def status_for(name: str) -> str:
 ...
 if code == "0":
 return "pass"
 if code == "blocked" or (code == "127" and name in {"composer-validate", "composer-install", "phpunit", "syntax"}):
 return "blocked"
 return "fail"
...
def build_test_report() -> None:
 ...
 ("syntax", "find src tests -name '*.php' -print0 | xargs -0 -n1 php -l"),
 ("evidence", "real Ariada CLI scan + screenshot"),
 ]
 rows = "\n".join(
 ...
 body = f"""
<p>Focused local gates for the Symfony bundle. PHP and Composer are required
for the framework gates; this host did not have a PHP runtime at build time, so
those gates are marked with the captured environment blocker.</p>
<table><thead><tr>
...
"""
 ...
 (TEST_REPORT / "result.html").write_text(page("Ariada Symfony test report", body), encoding="utf-8")
...
def build_scan_preview() -> None:
 ...
 body = f"""
<p>Real Ariada CLI scan against a representative rendered Symfony checkout route
fixture served over localhost.</p>
...
"""
 ...
 page("Ariada Symfony real scan preview", body),
 encoding="utf-8",
)
...
def build_scan_report() -> None:
 ...
 shot = (
 "<figure><a href='screenshots/scan-result.png'>"
 "<img alt='Screenshot of the Ariada Symfony scan result' "
 f"src='data:image/png;base64,{encoded}'></a><figcaption>"
 "Browser screenshot of the real scan result preview. Open the linked "
 "PNG to zoom.</figcaption></figure>"
)
 else:
 shot = "<p><strong>Evidence gap:</strong> screenshot file was not produced.</p>"
 body = f"""
<p>Symfony is a PHP web framework commonly used for enterprise and agency web
applications. This bundle lets Symfony teams add repeatable Ariada scan evidence
to release review without replacing Symfony or rewriting application views.</p>
<h2>What was implemented</h2>
<ul>
<li>Symfony bundle class and dependency-injection configuration.</li>
<li><code>bin/console ariada:scan {{url?}}</code> command.</li>
<li>Thin runner over shared <code>@ariada-org/cli</code>; no scanner logic is reimplemented.</li>
<li>PHPUnit tests for command behavior and CLI argument construction.</li>
<li>Representative Symfony route fixture and real Ariada CLI scan evidence.</li>
</ul>
<h2>Evidence screenshot</h2>
{shot}
<h2>Scan result</h2>
<p><strong>{total}</strong> finding(s) were reported by the shared scanner CLI.</p>
<p>Raw JSON: <a href='ariada-output/multi-domain-report.json'>ariada-output/multi-domain-report.json</a></p>
<p>Command log: <a href='command.log'>command.log</a></p>
<pre>{esc(read(SCAN_EVIDENCE / "command.log") or "(no command output)")}</pre>
<h2>Test adequacy</h2>
<p>The scan proves that the S99 evidence fixture is a real browser-captured
surface scanned through the shared Ariada CLI. PHP/Symfony runtime tests are
authored but could not be executed on this host because <code>php</code> and
<code>composer</code> were not installed.</p>
<h2>Human blockers</h2>
<p>Packagist publication requires the maintainer-owned Packagist account and
release credentials. Local PHP framework gates require a host with PHP 8.1+
and Composer.</p>
"""
 (SCAN_EVIDENCE / "result.html").write_text(
 page("Ariada Symfony scan evidence", body),
 encoding="utf-8",
)
