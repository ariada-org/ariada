...
/**
 * The canonical grid: findings by domain, within site. Two levels, and a level
 * that is not what it should be is skipped rather than refused — a partly
 * written grid should yield the findings it does hold, not none.
 */
function rowsFromGrid(grid) {
  const rows = [];
  for (const site of Object.values(grid)) {
...
export function normalizeAriadaFindings(report) {
  ...
  if (report.grid && typeof report.grid === 'object') return rowsFromGrid(report.grid);
  return [];
}
