...
const source = readFileSync(resolve(root, 'src/ServiceProvider.php'), 'utf8');

const failures = [];

if (composer.type !== 'statamic-addon') {
  failures.push(`the package type must be statamic-addon, not ${composer.type}`);
}
...
if (!source.includes("'statamic.entry-action'")) {
  failures.push("the scan request must be attributed to statamic.entry-action");
}
