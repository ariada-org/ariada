...
function checkWith(edits = {}) {
  const dir = mkdtempSync(join(tmpdir(), 'statamic-ariada-'));
...
test('the package as it stands is accepted', () => {
  ...
  assert.match(out, /PASS statamic-ariada structure/);
});

test('a package type the host does not recognise is refused', () => {
  const { status, out } = checkWith({
    'composer.json': (s) => s.replace('"statamic-addon"', '"library"'),
  });
...
test('a namespace the autoloader and the source disagree on is refused', () => {
  const { status, out } = checkWith({
    'src/ServiceProvider.php': (s) => s.replace(/^namespace .+;$/m, 'namespace Elsewhere;'),
  });
  assert.equal(status, 1);
...
test('mentioning a method instead of declaring it is refused', () => {
  const { status, out } = checkWith({
    'src/ServiceProvider.php': (s) => s.replace('function scanRequest(', '// function scanRequest was here: scanRequest('),
  });
  assert.equal(status, 1);
...
test('renaming what the scans are attributed to is refused', () => {
  ...
  const { status, out } = checkWith({
    'src/ServiceProvider.php': (s) => s.replace("'statamic.entry-action'", "'entry'"),
  });
  assert.equal(status, 1);
  assert.match(out, /statamic\.entry-action/);
});
