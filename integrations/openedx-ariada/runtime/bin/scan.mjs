...
async function probe() {
  ...
  process.stdout.write(JSON.stringify({ resolved, cli: "runScan", analyzer: "createA11yAnalyzer" }));
}
...
function parseArguments(argv) {
  ...
  if (argv.length === 0 || argv[0].startsWith("--")) {
    fail("Usage: ariada-openedx-scan <url> --output-dir <path> [options]");
  }
  const options = {
    probe: false,
    url: argv[0],
...
async function configureBrowser(options) {
  const storageState = process.env.OPENEDX_ARIADA_STORAGE_STATE || "";
  if (storageState) await access(storageState);
  if (!options.browserChannel && !storageState) return;
