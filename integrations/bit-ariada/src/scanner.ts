...
//
// THE SCAN IS ALWAYS RUN AT THE LOWEST THRESHOLD, AND THE GATE'S THRESHOLD IS
// APPLIED AFTERWARDS. So the stored report holds everything that was found, and
// raising or lowering what counts as a failure later needs no rescan — the
// artifact from a passing run is still a complete record rather than a blank.
//
// The output directory is emptied before the run rather than written over. A
// scan that dies before writing leaves nothing, which is honest; leaving the
// previous component's report there would attribute one component's findings to
// another.
//
// Output is captured with a two-megabyte ceiling that destroys the stream rather
// than truncating it, because this text ends up inside an error message and half
// a diagnostic is worse than a stated limit.
//
// The scanner is reached through its real entry point and the failure is
// explicit if that entry point is not there. This integration claims to run the
// actual scanner rather than an approximation of it, and a claim like that has
// to break loudly when it stops being true.
...
export async function scanRenderedPage(url: string, outputDir: string, timeoutMs: number): Promise<ParsedAriadaScan> {
  ...
  const exitCode = await runScan(url, { outputDir, browser: 'chromium', format: 'json', severityThreshold: 'minor', timeoutMs }, stdout.stream, stderr.stream, realCoreScan);
...
const realCoreScan = async (url: string, options: Record<string, unknown>) => {
  const [core, rules] = await Promise.all([import('@ariada-org/core-playwright'), import('@ariada-org/rules-axe')]);
