...
function validFixture() {
  ...
  const resolution = {
    ...
    productionApprovals: [{ actor: "predopta", state: "approved", environment: PRODUCTION_ENVIRONMENT }],
...
test("production approval parser accepts the real environments-array API contract", () => {
  const history = [{
    ...
    environments: [{
      ...
      name: PRODUCTION_ENVIRONMENT,
      url: `https://api.github.com/repos/${REPOSITORY}/environments/${PRODUCTION_ENVIRONMENT}`,
      html_url: `https://github.com/${REPOSITORY}/deployments/activity_log?environments_filter=${PRODUCTION_ENVIRONMENT}`,
    }],
  ...
  }]), /ariada-wiki-production approvals are absent/u);
  assert.throws(() => normalizeProductionApprovals([{
    state: "approved",
...
test("workflow has stable checks, complete governance suites, pinned actions, and no deployment", async () => {
  ...
  assert.match(workflow, /environment:\s*\n\s*name: ariada-wiki-production\s*\n\s*deployment: false/u);
...
test("release documentation describes post-merge identities and current components", async () => {
  ...
  assert.match(docs, /ariada-wiki-production/u);
