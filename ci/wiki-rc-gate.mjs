...
export const PROMOTION_APPROVAL_ENVIRONMENT = "ariada-wiki-promotion-approval";
...
function validatePromotionApprovalEnvironment(value) {
  const environment = record(value, "promotion approval environment");
  exact(environment.name, PROMOTION_APPROVAL_ENVIRONMENT, "promotion approval environment name");
  if (!Array.isArray(environment.protection_rules)) fail("promotion approval environment protection rules are absent");
  const wait = requireUniqueCandidate(environment.protection_rules.filter((rule) => rule?.type === "wait_timer"), "promotion approval wait timer rule");
  exact(wait.wait_timer, PRODUCTION_WAIT_TIMER_MINUTES, "promotion approval wait timer");
  ...
  if (!Array.isArray(reviewers.reviewers)) fail("promotion approval required reviewers are absent");
  const normalizedReviewers = reviewers.reviewers.map((entry) => {
    exact(entry.type, "User", "production reviewer type");
    return actor(entry.reviewer, "promotion approval required reviewer");
  });
  const required = requireUniqueCandidate(normalizedReviewers, "promotion approval required reviewer");
  if (!sameActor(required, REQUIRED_PRODUCTION_REVIEWER)) fail("promotion approval required reviewer is not predopta");
  const policy = record(environment.deployment_branch_policy, "production deployment branch policy");
  exact(policy.protected_branches, true, "promotion approval protected branch policy");
  exact(policy.custom_branch_policies, false, "promotion approval custom branch policy");
  return {
...
function validateNormalizedEnvironment(value) {
  const environment = exactKeys(
    ...
    "normalized promotion approval environment",
  );
  exact(environment.name, PROMOTION_APPROVAL_ENVIRONMENT, "normalized promotion approval environment name");
  exact(environment.waitTimerMinutes, PRODUCTION_WAIT_TIMER_MINUTES, "normalized promotion approval wait timer");
...
export function normalizeProductionApprovals(value) {
  ...
  for (const [index, entryValue] of value.entries()) {
    ...
    const matching = entry.environments.filter((environmentValue, environmentIndex) => {
      ...
      return canonicalString(environment.name, `production approval history ${index} environment ${environmentIndex} name`) === PROMOTION_APPROVAL_ENVIRONMENT;
    });
    if (matching.length > 1) fail(`production approval history ${index} contains duplicate promotion approval environments`);
  ...
  if (approvals.length === 0) fail(`current ${PROMOTION_APPROVAL_ENVIRONMENT} approvals are absent`);
  return approvals;
}
...
export function validateApprovalReviews(approvals, excludedActors) {
  ...
  const approved = approvals.filter((approval) => {
    ...
    exact(value.environment, PROMOTION_APPROVAL_ENVIRONMENT, "production approval environment");
...
async function resolvePromotionCommand(values, dependencies = {}) {
  ...
  const productionEnvironment = validatePromotionApprovalEnvironment(
    await client.json(`/repos/${repository}/environments/${encodeURIComponent(PROMOTION_APPROVAL_ENVIRONMENT)}`, "promotion approval environment configuration"),
  );
