...
- **Positive:** per-rule traceability to primary standards documents; auditable by procurement reviewers and accessibility auditors; false-positive and false-negative rates are accountable against normative text rather than paraphrase.
...
- Automated citation checking: not implemented in the current iteration; the human reviewer protocol is the primary gate. Automated citation verification (crawling WCAG 2.2 API + EN 301 549 machine-readable form) is planned for a later iteration.
...
- `packages/wcag-rules-extended/docs/rules/` — per-rule documentation stubs (current iteration)
- `@ariada-org/regulatory-mappings` package — machine-readable WCAG SC → EN 301 549 → EAA Annex I mapping table (planned for a later iteration; not yet published)
